...
CREATE TABLE `user` (
  ...
  `githubUsername` varchar(50) DEFAULT NULL COMMENT 'githubUername of user',
  `githubAuthToken` varchar(250) DEFAULT NULL COMMENT 'Github auth Token with the required access for repo',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `user_githubUsername_unique` (`githubUsername`,`isDeleted`)
...
CREATE TABLE `commit` (
  ...
  `commitMessage` varchar(250) DEFAULT NULL COMMENT 'commit message',
  `createdAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `isActive` tinyint(4) DEFAULT 1,
  `isDeleted` int(10) unsigned DEFAULT 0 COMMENT '0 means not deleted, we will copy id in this field to mark record as deleted',
